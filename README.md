...
# Final submission checklist (Week 6 GitHub repo)

* [ ] `Week6/README.md` (this file) — include tool versions and PDK commit
* [ ] `Day1/` to `Day5/` lecture-notes and lab-scripts
* [ ] `Designs/` — example RTL and config for at least one small design (e.g., inverter-based test or picoRV32)
* [ ] `Magic/` — sample cell `.mag` and extracted `.spice`
* [ ] `Reports/` — synthesis, floorplan, placement, CTS, route, DRC logs
* [ ] `dashboard.md` — progress & notes

---

# Minimal dashboard (copy to dashboard.md)
...
## Optional extras you may want to add to the repo

* `Makefile` or a root `run_all.sh` to reproduce the main flow.
* `versions.txt` listing exact commits and docker images used.
* A `video_tutorials.md` or short screencast links for critical steps (Magic view, running ngspice, OpenSTA reports).

---

## Final tips for reviewers and graders

* Provide clear commit history and small commits for each lab step.
* For DRC fixes, include an image or screenshot and a short note describing the change.
* Explain why you picked the utilization/aspect ratio and include a short paragraph comparing two different choices if you experimented.

---

*Need a PDF export, separate lab-by-lab detailed step sheets, or a PR-ready GitHub structure?  Tell me which and I can generate the files and an export-ready layout.*
